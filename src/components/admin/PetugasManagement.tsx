...
import { Add, Edit, Delete, Search, ManageAccounts, Close } from '@mui/icons-material';
...
export default function PetugasManagement() {
  ...
  const [form, setForm] = useState({ username: '', password: '', nama_petugas: '', alamat: '', telp: '' });
  const [deleteId, setDeleteId] = useState<number | null>(null);
  const [snack, setSnack] = useState({ open: false, msg: '', sev: 'success' as 'success' | 'error' });
  const [saving, setSaving] = useState(false);

  const load = async () => { setLoading(true); try { const r = await apiFetch('/petugas'); setData(Array.isArray(r) ? r : []); } catch {} finally { setLoading(false); } };
  useEffect(() => { load(); }, []);

  const handleOpen = (item?: any) => {
    if (item) { setEditItem(item); setForm({ username: '', password: '', nama_petugas: item.nama_petugas || '', alamat: item.alamat || '', telp: item.telp || '' }); }
    else { setEditItem(null); setForm({ username: '', password: '', nama_petugas: '', alamat: '', telp: '' }); }
    setOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        const body = { nama_petugas: form.nama_petugas, alamat: form.alamat, telp: form.telp };
        await apiFetch(`/petugas/${editItem.id}`, { method: 'PATCH', body: JSON.stringify(body) });
        setSnack({ open: true, msg: 'Petugas berhasil diupdate', sev: 'success' });
      } else {
        if (!form.username || !form.password || !form.nama_petugas) { setSnack({ open: true, msg: 'Semua field wajib diisi', sev: 'error' }); setSaving(false); return; }
        await apiFetch('/petugas', { method: 'POST', body: JSON.stringify(form) });
        setSnack({ open: true, msg: 'Petugas berhasil ditambahkan', sev: 'success' });
      }
      setOpen(false); load();
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try { await apiFetch(`/petugas/${deleteId}`, { method: 'DELETE' }); setSnack({ open: true, msg: 'Petugas berhasil dihapus', sev: 'success' }); load(); }
    catch (e: any) { setSnack({ open: true, msg: e.message, sev: 'error' }); }
    setDeleteId(null);
  };

  const filtered = data.filter(d => d.nama_petugas?.toLowerCase().includes(search.toLowerCase()));

  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          <Typography sx={{ color: 'white', fontWeight: 700, fontSize: 20 }}>Data Petugas</Typography>
          <Typography sx={{ color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>{data.length} petugas terdaftar</Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
          <TextField size="small" placeholder="Cari petugas..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      <Card sx={cs}>
        <TableContainer><Table>
          <TableHead><TableRow>
            <TableCell sx={hc}>ID</TableCell><TableCell sx={hc}>Nama Petugas</TableCell><TableCell sx={hc}>Alamat</TableCell><TableCell sx={hc}>Telepon</TableCell><TableCell sx={hc} align="right">Aksi</TableCell>
          </TableRow></TableHead>
          <TableBody>
            ...
            : filtered.map(item => (
              <TableRow key={item.id} sx={{ '&:hover': { background: 'rgba(255,255,255,0.02)' } }}>
                ...
                <TableCell sx={bc}><Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  <Box sx={{ width: 36, height: 36, borderRadius: 2, background: 'rgba(240,147,251,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ManageAccounts sx={{ color: '#f093fb', fontSize: 18 }} /></Box>
                  <Typography sx={{ color: 'white', fontWeight: 600, fontSize: 14 }}>{item.nama_petugas}</Typography>
                </Box></TableCell>
                <TableCell sx={{ ...bc, color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>{item.alamat}</TableCell>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: dp }}>
        <DialogTitle sx={{ color: 'white', fontWeight: 700, display: 'flex', justifyContent: 'space-between' }}>{editItem ? 'Edit Petugas' : 'Tambah Petugas'}<IconButton onClick={() => setOpen(false)} sx={{ color: 'rgba(255,255,255,0.4)' }}><Close /></IconButton></DialogTitle>
        <DialogContent sx={{ pt: '16px !important', display: 'flex', flexDirection: 'column', gap: 2.5 }}>
          ...
          <TextField label="Nama Petugas" value={form.nama_petugas} onChange={e => setForm({ ...form, nama_petugas: e.target.value })} fullWidth sx={tf} />
      ...
      <Dialog open={!!deleteId} onClose={() => setDeleteId(null)} PaperProps={{ sx: dp }}>
        ...
        <DialogContent><Typography sx={{ color: 'rgba(255,255,255,0.6)' }}>Yakin ingin menghapus petugas ini?</Typography></DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
